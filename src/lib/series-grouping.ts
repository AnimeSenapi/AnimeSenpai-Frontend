...
export function extractSeriesInfo(
  ...
  const patterns = [
    // Final/Last seasons
    {
      regex: /:\s*Final Season\s*(?:Part\s*(\d+))?/i,
    ...
    { regex: /\sS(\d+)/i, number: (m: RegExpMatchArray) => parseInt(m[1] ?? '1') },
    ...
    // Numbers at the end
    { regex: /:\s*(\d+)$/i, number: (m: RegExpMatchArray) => parseInt(m[1] ?? '1') },
    { regex: /\s+(\d+)$/i, number: (m: RegExpMatchArray) => parseInt(m[1] ?? '1') },
...
export function groupAnimeIntoSeries(animeList: any[]): any[] {
  ...
  for (const anime of animeList) {
    ...
    const normalizedName = seriesName
